export default function dragNode(event, target, parent, grid, scale){
    ...
    const handleMouseMove = (ev) => {
        current.x = Math.round(((ev.clientX + bounding.x)/ scale )/ grid) * grid
        current.y = Math.round(((ev.clientY + bounding.y) / scale) / grid) * grid
