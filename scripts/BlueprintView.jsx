...
export default function BlueprintView(props) {
    const {submitPackage, engine, id, name} = props
    ...
    return (<div className={styles.prototypeWrapper} ref={ref} id={id + '-board-wrapper'}>
        <Structure
            hook={hook}
        ...
        <div ref={wrapperRef} className={styles.prototypeWrapperBoard}>
            <Board
                ...
                onDrop={(ev) => {
                    ...
                    if (entity) return [true, new EntityReference(dt, entity?.name, Object.keys(entity.components))]
                    return [true]
                }}
                hook={hook}
        ...
        <ResizableBar type={'width'}/>
        <div style={{
            display: 'flex', flexDirection: 'column', width: '275px', gap: '3px', overflow: 'hidden'
        }}>
            <NodeEditor
                hook={hook}
                selected={hook.selected}
...
BlueprintView.propTypes = {
    engine: PropTypes.object, id: PropTypes.string, name: PropTypes.string, submitPackage: PropTypes.func.isRequired
}